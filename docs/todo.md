# Matchmaking Studio - 可执行开发清单

> 依据：[plan.md](./plan.md)  
> 目标：把当前项目收束为一套 AI-first、红娘主导、面向高质量客户的婚恋撮合工作台  
> 使用规则：只有在“代码已落地 + 本地验证通过 + 关键页面/流程实际可用”后，才将对应任务勾选为已完成

---

## 0. 当前已完成基线

- [x] 项目已经迁移到 `Next.js 16 + App Router + Supabase + Tailwind CSS + shadcn/ui`
- [x] 本地环境已经可用，`npm run build` 可通过
- [x] 代码仓库已经上传到 GitHub，并已更名为 `matchmaking-studio`
- [x] 产品与技术总方案已经写入 [plan.md](./plan.md)
- [x] 已有红娘端、管理端、用户端基础路由与认证保护
- [x] 已有音频上传、转录、结构化提取、写库、匹配、提醒的基础闭环
- [x] 已有 AI-first 的基础字段同步能力和异常确认能力
- [x] 已有本地测试账号、模拟角色与示例录音数据

> 说明：从这里开始，后续 todo 不再把“能跑起来”当目标，而是按新版 `plan` 逐步重构为更稳定、更优雅、更高质量的产品形态。

---

## 1. Phase UI-0：Apple 式主题重置与全局壳层基线

> 说明：本阶段不是做“局部美化”，而是正式放弃旧的暖金、深棕、私享顾问室路线，把产品整体切到更年轻、明亮、轻盈、顺滑的 Apple 式轻工作台方向。  
> 目标：先把整个系统的“空气”和壳层换掉，再进入页面级重构。

### 1.1 品牌与命名统一

- [x] GitHub 仓库名统一为 `matchmaking-studio`
- [x] `package.json` / `package-lock.json` 名称统一为 `matchmaking-studio`
- [x] 应用标题统一为 `Matchmaking Studio`
- [x] 登录页、侧边栏、README、计划文档的品牌名称统一
- [x] 检查并清理页面中剩余旧品牌文案与旧平台描述

### 1.2 旧视觉体系止损与审计

- [ ] 盘点当前界面中仍残留的“暖金 / 深棕 / 会所感 / 礼宾感”来源
- [ ] 盘点 sidebar、top bar、card、badge 中所有与新方向冲突的硬编码颜色
- [ ] 盘点仍依赖重阴影、彩色描边、厚圆角制造存在感的组件
- [ ] 盘点高饱和 badge 与彩虹式状态色的使用位置
- [ ] 盘点 `globals.css` 中仍偏旧路线的 token

### 1.3 新主题 token 设计与落地

- [ ] 基于 [TweakCN](https://tweakcn.com/editor/theme?p=dashboard) 试出 Apple 式轻工作台 token 草案
- [ ] 重做页面背景（Canvas）层级：`background / section background / page background`
- [ ] 重做面板（Surface）层级：`surface-1 / surface-2 / surface-3 / card / popover`
- [ ] 重做文字层级：`text-primary / text-secondary / text-tertiary`
- [ ] 重做语义状态色：`neutral / attention / success / risk`
- [ ] 重做 sidebar 专用 token：背景、hover、active、border、icon tone
- [ ] 重做 border / ring / radius / shadow 规则
- [ ] 将新 token 全量回写到 `app/globals.css`
- [ ] 删除或替换旧暖金深棕路线的残留 token

### 1.4 全局骨架重构

- [ ] 重构左侧主导航（Left Rail），从“深棕主视觉墙”改成“安静导航层”
- [ ] 重构顶部上下文条（Top Context Bar），从“礼宾标题条”改成“上下文工具条”
- [ ] 重构主工作区骨架（Main Worksurface），让中央内容成为主角
- [ ] 重构可复用的右侧辅助栏（Assist Rail），只承载当前任务强相关信息
- [ ] 统一页面容器宽度、栅格、section 间距与留白节奏
- [ ] 降低无意义的卡片嵌套层级
- [ ] 确保移动端和中小屏不会因为固定布局直接崩坏

### 1.5 基础组件系统统一

- [ ] 重做 `Button` 的默认、次级、幽灵、危险态视觉语言
- [ ] 重做 `Badge`，降低高饱和状态色使用频率，避免彩虹式 badge
- [ ] 重做 `Card` 的边框、阴影、背景与圆角规则
- [ ] 重做 `Dialog / Sheet` 的材质与滚动容器样式
- [ ] 重做 `Tabs` 的激活态与分组层级
- [ ] 重做 `Input / Textarea / Select` 的系统控件气质
- [x] 全站统一使用 `lucide-react`
- [ ] 清理页面里直接散落的硬编码颜色类名

### 1.6 前端展示语义收敛

- [x] tri-state 字段前端不再直接显示 `unknown / yes / no` 等数据库原值
- [x] 客户详情编辑态中，未确认 tri-state 字段默认显示 `请选择`，而不是预选 `unknown`
- [x] 客户详情只读态中，`unknown` 统一展示为 `待确认`，空值统一展示为 `未填写`
- [x] V1 后置的特殊关系模式字段从常规客户详情主区降级，不再与通用信息同优先级主显

### 1.7 Phase UI-0 验收

- [ ] 用户第一眼不再把产品认成深棕婚介后台
- [ ] sidebar 不再成为视觉主角
- [ ] top bar 更像上下文工具条，而不是品牌展示条
- [ ] card 体系明显变轻，不再有厚重奶油卡感
- [ ] 页面整体亮度、呼吸感和连续浏览舒适度明显提升

---

## 2. Phase 1：字段系统与数据库治理

### 2.1 字段字典与来源治理

- [x] 审核当前 `field-spec` 与 [plan.md](./plan.md) 的一致性
- [x] 为所有 V1 核心字段补齐：
- [x] 字段 key
- [x] 中文名
- [x] 类型
- [x] 允许值
- [x] 敏感性
- [x] 匹配层级（硬筛选 / 软偏好 / 待确认 / 参考）
- [x] 提取规则
- [x] 更新规则
- [x] 证据要求
- [x] 明确哪些字段只允许客户明确表达后写入
- [x] 明确哪些字段允许 AI 做保守总结

### 2.2 数据模型审计与补齐

- [x] 审核 `profiles` 是否完整覆盖 V1 基础事实字段
- [x] 审核 `intentions` 是否完整覆盖关系模式和三态接受状态
- [x] 审核 `trait_profiles` 是否完整覆盖 V1 生活方式字段
- [x] 审核 `matches` 是否完整覆盖 `confirmed / pending_confirmation / rejected`
- [x] 审核 `followup_tasks` 是否能承载缺口补问与敏感确认
- [x] 新增 `field_observations` 表，用于字段级来源与证据追溯
- [x] 为关键字段补充 `source_type / confidence / verification_status / conversation_id`

### 2.3 索引、RLS 与兼容迁移

- [x] 为 tri-state、relationship mode、recommendation type 补足索引
- [x] 为 `followup_tasks(status, matchmaker_id)` 补足索引
- [x] 为 `field_observations(profile_id, field_key)` 补足索引
- [x] 为新增表补齐 RLS
- [x] 对历史数据补齐默认值与兼容迁移
- [x] 更新数据库类型生成与 TypeScript 类型

---

## 3. Phase 2：AI 字段提取协议与增量更新链路

### 3.1 提取协议落地

- [x] 审核当前提取 prompt 是否完全对齐 [plan.md](./plan.md) 中的 `AI 字段提取协议 v1`
- [x] 强制模型输出 patch，而不是完整档案对象
- [x] 强制模型输出 `field_updates`
- [x] 强制模型输出 `review_required`
- [x] 强制模型输出 `missing_critical_fields`
- [x] 强制模型输出 `suggested_followup_questions`
- [x] 强制模型输出 `summary_updates`
- [x] 强制模型输出 `processing_notes`

### 3.2 输入包完善

- [x] 保证提取时传入“当前客户快照”
- [x] 传入会话系统上下文：`profile_id / profile_gender / matchmaker_id / conversation_id / talked_at / uploaded_at`
- [x] 若可用则传入 `transcript_verbose_json`
- [x] 传入字段说明书的机器可读版本
- [x] 只传必要上下文，控制 token 体积

### 3.3 Patch 应用规则固化

- [x] 确保“未提及字段不覆盖旧值”
- [x] 确保“模糊新值不覆盖明确旧值”
- [x] 确保 `unknown` 不覆盖 `yes / no` 或明确事实
- [x] 确保敏感字段必须尽量带 `evidence_excerpt`
- [x] 确保冲突字段进入异常确认，而不是默默覆盖
- [x] 确保可自动写入字段默认直写入库
- [x] 确保 `summary_updates` 只允许白名单字段

### 3.4 字段来源与历史记录

- [x] 每次提取后保留本次原始 patch
- [x] 将本次 patch 与最终生效字段做差异记录
- [x] 把关键字段同步写入 `field_observations`
- [x] 支持按字段回看“来自哪次录音、哪段证据”

### 3.5 错误处理与容错

- [x] 统一处理云雾 Whisper 错误
- [x] 统一处理云雾 Claude 错误
- [x] 用户侧错误文案必须短句化
- [x] 合同解析失败时不直接把原始 JSON 暴露到前端
- [ ] 统计未知字段、别名漂移和模型输出异常，便于后续迭代

---

## 4. Phase 3：AI-first 录音与建档主流程

### 4.1 上传与转录链路

- [x] 审核上传页是否真正以“录音优先”而不是“表单优先”
- [x] 确保音频上传写入 `conversations`
- [x] 确保记录负责红娘、客户、上传时间、音频时长
- [x] 确保 `talked_at` 有合理自动推断逻辑
- [x] 为 Storage 下载补齐重试与超时处理
- [x] 将浏览器端音频上传改为 Supabase resumable upload，并显示资料库上传进度
- [x] 为转录请求补齐幂等保护
- [x] 将转录主链路改为：`Storage signed URL -> Groq`
- [x] 服务端生成短时效签名 URL，而不是默认下载整段文件后再转传
- [x] Groq 转录失败时，自动切换到云雾 Whisper 兜底
- [x] 上传阶段与转录阶段在状态机与接口实现上彻底解耦
- [x] 为 `conversations.status` 增加并落地中间状态：`uploaded`、`transcribed`
- [x] 为失败会话增加 `failed_stage` 语义，明确区分 `upload / transcribe / extract`
- [x] 让“上传录音音频到对象存储”在实现上只负责把会话推进到 `uploaded`
- [x] 让 `/api/transcribe` 只负责把 `uploaded -> transcribed`，不再顺手推进提取
- [x] 让 `/api/extract` 只接受 `transcribed` 会话，并独立完成 `transcribed -> done`
- [ ] 将前端上传页从“驱动整条处理链”改为“展示状态 + 提供分阶段重试入口”
- [ ] 将重试动作拆为：重新上传音频 / 重试转录 / 重试提取
- [x] 前端步骤文案明确区分“上传到资料库中”与“发送到转录服务中”
- [x] 确保同一段已入库音频重试转录时无需重新上传
- [x] 为 URL 转录补齐真实联通回归验证

### 4.2 提取与自动写库

- [x] 确保转录成功后自动进入结构化提取
- [x] 确保提取成功后自动写入客户当前档案
- [x] 确保写入完成后会话状态变为 `done`
- [x] 确保敏感字段与冲突字段进入异常确认链路
- [x] 确保提取后的缺口与补问写回 `conversations`

### 4.3 新客户建档体验

- [x] 支持“最少上下文创建草稿客户 + 直接上传第一段音频”
- [x] AI 自动补齐第一段录音中的基础事实字段
- [x] 只在重名、身份冲突、归属不清时要求红娘处理
- [x] 优化上传完成后的成功态、失败态、恢复态

### 4.4 多轮录音增量更新

- [x] 验证“第二段录音可更新第一段录音建立的档案”
- [x] 验证 tri-state 可从 `unknown -> yes/no`
- [x] 验证明确新值可覆盖旧值
- [x] 验证模糊新值不会污染旧值
- [x] 验证历史录音与当前档案可同时保留

### 4.5 长音频策略（V1 边界）

- [x] 明确 V1 默认整段转录，不引入自动分段
- [ ] 为上传页和后端增加“当前版本暂不做分段”的实现注释与文档说明
- [ ] 对超大文件只做大小限制与清晰提示，不提前实现 chunking
- [x] 将“静音切段 / overlap / 合并去重”移入后续版本储备

---

## 5. Phase 4：匹配引擎与待确认候选机制

### 5.1 匹配主流程

- [x] 审核基础匹配打分是否覆盖 V1 核心字段
- [x] 加入关系模式覆盖层
- [x] 明确 `confirmed / pending_confirmation / rejected` 三类输出
- [x] 确保 `unknown != no`
- [x] 确保 `no` 直接排除
- [x] 确保 `yes` 可进入已确认候选
- [x] 确保 `unknown` 进入待确认候选

### 5.2 偏好权重

- [x] 为年龄、城市、学历、收入、婚史、孩子、生育、生活方式、关系模式接入重要性等级
- [x] 固定四档：`hard / important / normal / flexible`
- [x] 将语言线索映射到重要性等级
- [x] 在得分中体现“满足程度 × 重要性权重”

### 5.3 候选解释

- [x] 输出总分
- [x] 输出分字段得分
- [x] 输出硬冲突字段
- [x] 输出待确认字段
- [x] 输出“为什么排前面”
- [x] 输出“下一步应该问什么”

### 5.4 重跑策略

- [x] 当匹配关键字段变更时自动重跑匹配
- [x] 当 tri-state 从 `unknown` 变为 `yes/no` 时自动重跑
- [x] 当关系模式发生变化时自动重跑
- [x] 保证重跑不会把已有跟进状态误重置

---

## 6. Phase 5：补问任务与提醒闭环

### 6.1 自动补问任务

- [x] 将 `missing_critical_fields` 可靠转成 `followup_tasks`
- [x] 将敏感待确认项转成 `sensitive_confirmation` 任务
- [x] 将任务绑定到客户、匹配、红娘
- [x] 为任务生成一问一事的补问列表

### 6.2 提醒中心联动

- [x] 将 `pending_confirmation` 长时间未处理接入提醒
- [x] 将长期无新录音客户接入提醒
- [x] 将长期无跟进匹配接入提醒
- [x] 将提醒与 `followup_tasks` 打通

### 6.3 补问完成后的回流

- [x] 当红娘上传后续录音时，自动尝试关闭相关补问任务
- [x] 当 tri-state 被明确后，自动关闭对应确认任务
- [x] 在匹配页和提醒中心同步刷新状态

---

## 7. Phase UI-1：红娘核心工作流页面重构

> 说明：本阶段目标不是“把几个页面做漂亮”，而是把红娘最常用的页面真正做成工作台。  
> 标准：红娘进入页面后，3 秒内要知道“谁 / 现在什么状态 / 下一步做什么”。

### 7.1 客户详情页骨架

- [ ] 重构客户头部 Hero 区，改成更轻的识别与状态区
- [ ] 让头部只展示最关键的 6-8 项信息
- [ ] 统一头部中的头像、姓名、年龄、城市、当前阶段、最近更新时间层级
- [ ] 统一头部快捷动作的位置、样式和重要性顺序
- [ ] 固定右侧辅助栏：最近更新 / 缺口 / 待确认 / 下一步
- [ ] 统一 tab 导航层级与布局
- [ ] 清理头部中所有不必要的重色块、厚边框和强 badge
- [x] 将客户头像与生活照分离建模：头像单独用于识别，生活照单独用于资料展示
- [x] 客户详情页头部主图区显示头像，而不是依赖名字识别客户
- [x] 客户详情页资料区新增“生活照”模块，承载多张补充照片
- [x] 客户详情编辑区将“照片 URL”改为两个入口：上传头像 / 上传生活照

### 7.2 录音记录页

- [ ] 将录音列表改造成更轻的“时间线摘要卡”
- [ ] 明确拆分：录音速记 vs AI 摘要
- [ ] 明确显示本次新增 / 异常 / 缺口 / 状态
- [ ] 优化状态、数量、动作按钮的扫描效率
- [ ] 不再让长转录摘要被遮挡或硬截断得难以阅读
- [ ] 让红娘一眼判断“这条录音值不值得点开”
- [ ] 统一时间线卡片的视觉层次、边框、阴影和摘要节奏

### 7.3 录音详情工作台

- [ ] 将“查看详细”升级为双栏工作台式详情层
- [ ] 左侧只放转录全文与证据
- [ ] 右侧只放 AI 摘要、自动写入、异常、缺口、补问
- [ ] 修复转录全文无法独立滚动的问题
- [ ] 修复弹窗宽度、层级、滚动和信息压缩问题
- [ ] 统一“证据区 / 结论区 / 动作区”的视觉关系
- [ ] 降低顶部统计区的噪音，让主内容更容易读
- [ ] 清理详情层中任何仍像“数据容器”的厚重结构

### 7.4 审核页

- [ ] 将审核页彻底收束为“差异与异常处理页”
- [ ] 默认只展示变化、冲突、敏感项、低置信度项
- [ ] 显示当前值 vs AI 候选 vs 证据片段
- [ ] 三态字段必须用红娘可理解的交互控件
- [ ] 不再暴露原始 field key、英文枚举值、原始 JSON
- [ ] 让审核动作更像处理工作流，而不是填写表单
- [ ] 统一审核页证据区与处理区的层级关系

### 7.5 客户列表页

- [ ] 重构客户列表卡片，让列表首先服务“快速识别 + 快速判断”
- [x] 客户列表卡片左侧缩略图只使用头像
- [x] 没有头像时显示默认占位，不自动拿生活照顶替
- [x] 列表卡片与详情页头部的头像来源保持一致
- [ ] 卡片中姓名、状态、基础画像、最近更新形成稳定层级
- [ ] 降低列表卡片上的边框、阴影和装饰性存在感
- [ ] 统一“新增客户”按钮与筛选器区域的轻工作台风格
- [ ] 让列表页整体更像人物工作台，而不是营销卡片墙

### 7.6 Phase UI-1 验收

- [ ] 红娘进入客户详情页 3 秒内知道当前客户状态
- [ ] 录音列表一眼可判断“值不值得点开”
- [ ] 录音详情不再像窄长数据弹窗
- [ ] 审核页不再像大表单
- [ ] 客户列表卡片不再有厚重旧后台感

---

## 8. Phase UI-2：匹配与跟进工作台重构

> 说明：本阶段目标不是做“更炫的匹配页”，而是让红娘快速判断哪些候选能推、哪些候选要补问、哪些候选不值得继续。

### 8.1 匹配列表页

- [ ] 按 `confirmed / pending_confirmation` 明确分区
- [ ] 前台主显 Top1
- [ ] 保留 Top3 候选
- [ ] 每张卡片都显示“为什么值得看”
- [ ] `pending_confirmation` 卡片明确显示“缺什么、问什么、为什么值得继续”
- [ ] 让 `confirmed` 与 `pending_confirmation` 在视觉上有稳定语义差异
- [ ] 重新组织匹配卡片的信息顺序：人物 -> 结论 -> 风险 -> 下一步
- [ ] 去掉当前卡片里多余的粉边、重描边和噪音状态块

### 8.2 匹配详情页

- [ ] 强化匹配理由展示
- [ ] 强化待确认字段展示
- [ ] 强化下一步动作建议展示
- [ ] 让红娘可以快速做推进决策
- [ ] 把“为什么匹配 / 还缺什么 / 下一步干什么”拆成稳定区块
- [ ] 避免详情页重新变成字段堆叠容器

### 8.3 跟进记录页

- [ ] 统一跟进时间线样式
- [ ] 将跟进动作、结果、风险提示拆层
- [ ] 与补问任务、匹配状态联动
- [ ] 让时间线更像“推进记录”而不是日志列表

### 8.4 提醒中心

- [ ] 将提醒改造成真正的工作列表，而不是简单通知列表
- [ ] 支持按优先级、任务类型、状态过滤
- [ ] 支持从提醒直接跳到客户、匹配、补问任务
- [ ] 统一提醒卡片的紧急度表达，减少颜色噪音
- [ ] 让提醒中心更像任务面板，而不是通知流

### 8.5 Phase UI-2 验收

- [ ] 匹配页不再像分数榜单
- [ ] 红娘能快速区分“可立即推进”和“还需补问”
- [ ] 跟进页与提醒中心不再有明显的旧后台视觉断裂
- [ ] 页面整体保持 Apple 式轻工作台气质，不回退到旧色系和旧卡片语言

---

## 9. Phase UI-3：管理端治理与分析

> 说明：管理端是最后阶段，不应再反向影响红娘主工作流的优先级。  
> 目标：做成现代治理工作台，而不是传统深色运营后台。

### 9.1 管理看板

- [ ] 重构管理端 dashboard 的信息层级
- [ ] 展示字段完整度
- [ ] 展示敏感模式候选数量
- [ ] 展示待确认积压量
- [ ] 展示红娘补问完成率
- [ ] 展示匹配推进漏斗
- [ ] 统一指标卡、趋势图、漏斗图和筛选器的轻工作台语言
- [ ] 去掉任何重新回到传统深色后台的视觉惯性

### 9.2 客户与匹配治理页

- [ ] 提供字段完整度筛选
- [ ] 提供敏感关系模式筛选
- [ ] 提供待确认候选筛选
- [ ] 提供异常提取记录与失败记录筛选
- [x] 红娘端支持删除自己上传的客户，并同步清理关联录音与业务数据

### 9.3 运营与质量观察

- [ ] 增加录音处理成功率统计
- [ ] 增加提取失败率统计
- [ ] 增加未知字段比例统计
- [ ] 增加模型输出漂移观察点

### 9.4 Phase UI-3 验收

- [ ] 管理端看起来像治理工具，而不是旧式运营后台
- [ ] 图表、指标卡和筛选器形成统一视觉系统
- [ ] 管理端不会因为单独风格而破坏整站的一致性

---

## 10. 测试、联调与发布

### 10.1 回归测试

- [x] 为字段 patch 应用补齐回归测试
- [x] 为 tri-state 更新补齐回归测试
- [x] 为匹配分类补齐回归测试
- [x] 为补问任务生成补齐回归测试
- [x] 为用户侧短句错误提示补齐回归测试

### 10.2 真实流程联调

- [x] 用男方首轮录音验证自动建档
- [ ] 用女方首轮录音验证自动建档
- [x] 用补充录音验证增量更新
- [x] 验证 `unknown -> yes`
- [ ] 验证 `unknown -> no`
- [x] 验证 `pending_confirmation -> confirmed`
- [x] 验证异常确认页真实可用

### 10.3 UI 验收

- [x] 客户详情页在桌面端可用
- [ ] 客户详情页在移动端可用
- [x] 录音详情转录全文可独立滚动
- [x] 红娘前台不再看到原始英文枚举值
- [x] 录音列表摘要不再被遮挡
- [x] 全站图标统一为 `lucide-react`

### 10.4 发布准备

- [x] 清理 `.env` 说明与部署文档
- [ ] 校验生产数据库 migration 顺序
- [x] 校验 Vercel / Supabase 环境变量
- [x] 校验生产构建
- [x] 完成上线前 smoke test

---

## 11. V2 储备项（暂不进入当前主线）

- [ ] 说话人分离 / diarization
- [ ] 长音频自动分段
- [ ] overlap 切段与 transcript 合并去重
- [ ] 段首上下文补偿与术语 prompt
- [ ] MBTI 自报字段
- [ ] Big Five 问卷接入
- [ ] Attachment 问卷接入
- [ ] HEXACO 问卷接入
- [ ] 问卷式补录入口
- [ ] 自助客户端
- [ ] 更精细的财富与核验工作流

---

## 12. 当前阶段最优先事项

- [ ] 完成核心工作流稳定性任务：上传音频 -> 转录 -> 结构化提取 -> 写数据库
- [ ] 完成 Phase UI-0：Apple 式主题重置与全局壳层基线
- [x] 完成 Phase 1：字段系统与数据库治理审计补齐
- [ ] 完成 Phase 2：AI 字段提取协议与增量更新链路加固
- [ ] 完成 Phase UI-1：红娘核心工作流页面重构
- [x] 完成 Phase 4：匹配引擎与待确认候选机制稳定化

### 12.1 当前执行顺序（固定）

- [ ] 先完成核心工作流稳定性任务：上传音频 -> 转录 -> 结构化提取 -> 写数据库
- [ ] 再推进 Phase UI-0：先换掉全局壳层与主题空气
- [ ] 再推进 Phase UI-1：重构客户与录音主工作流页面
- [ ] 再推进 Phase UI-2：重构匹配与跟进工作台
- [ ] 最后推进 Phase UI-3：管理端治理与分析

> 说明：UI 重构不能再回到旧暖金深棕路线，也不能通过局部美化替代壳层与主题重建。所有页面改动都必须服从 [plan.md](./plan.md) 中的 Apple 式轻工作台方向。
