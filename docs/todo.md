...
> 当前执行边界：只覆盖红娘端桌面工作台。管理端、用户端和移动端在本轮中不继续扩写，统一后置到 roadmap。
...
- [x] 首批页面接入时，继续沿用当前项目自己的：
  ...
  - [x] role 判断
  - [x] 工作流逻辑
- [x] 明确本轮不再继续扩写管理端与用户端页面，避免 UI 重构范围失控
...
- [x] 将“新增客户”页接回当前设计系统，统一 hero、表单面板与创建动作样式
...
- [x] 为暗色模式补齐“上传录音”页的拖拽区、进度态、成功态与错误态层次
- [x] 为暗色模式补齐“录音审核”页的转录全文、自动同步、待补问与异常确认卡片系统
- [x] 修正暗色模式下“匹配工作台”和“匹配详情”页的匹配分数字、筛选胶囊与得分条对比度错误
- [x] 为暗色模式补齐“匹配详情”页男女资料对比卡的头像区、信息胶囊、兴趣爱好、意图区与 AI 描述层次
- [x] 清理“匹配详情”页资料卡在暗色模式下残留的浅底打印区、信息面板与兴趣标签材质
- [x] 统一“产品说明”页与通用 alert dialog / dropdown menu 的当前主题语言
- [x] 清理“匹配详情”页状态条、待确认面板与跟进操作区残留的旧色直写和暗色断层
- [x] 完成红娘端桌面最后一轮全页巡检，统一“匹配推荐 / 提醒中心 / 匹配卡”残留的白底面板与暗色弱对比控件
...
- [x] 当前 UI 交付范围收窄为红娘端桌面工作台；管理端、用户端与移动端统一放入 roadmap
