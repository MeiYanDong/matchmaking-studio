...
> 日期：2026-04-03  
> 核心定位：以红娘为操作主体、以字段体系为核心资产、以 AI 降低人工成本的高质量婚恋匹配平台
...
- 音频转录：`whisper-1`
  - 来源：`/v1/audio/transcriptions`
  - 当前文档明确写“目前只有 whisper-1 可用” [voice_api.md.md](./API/voice_api.md.md)
- 结构化提取与补问生成：Claude `messages`
  - 来源：`/v1/messages`
  - 当前网关文档暴露的是聊天接口而非强 schema 接口 [claude_api.md.md](./API/claude_api.md.md)
...
| 转录 | 云雾聚合网关的 OpenAI 兼容音频转写接口（当前文档为 `whisper-1`） |
| 结构化提取 | 云雾聚合网关的 Claude/OpenAI 兼容聊天接口 |
| 任务调度 | Next.js API / 定时任务 / 后续可接 Supabase Webhook |

### 15.2 第三方 API 选型说明

当前基于 `docs/API` 中的网关文档：

- 音频转文本优先使用 `whisper-1`
- Claude 继续承担结构化提取、补问生成、匹配理由生成
- 若后续网关支持更强的转录模型或说话人分离，可升级，但 V1 不以此为前提
