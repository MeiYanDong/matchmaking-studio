...
这次 POC 不追求替换线上整条链路，也不追求一开始就兼容所有远程环境。
...
**POC 先只验证“Agent 读 transcript 并直接写本地数据库”这一段。**
...
```mermaid
flowchart LR
  A["本地 transcript 样本"] --> B["Agent Runner"]
  B --> C["读取 profile / intention / trait 快照"]
  B --> D["读取最近几次会话摘要"]
  B --> E["直接更新本地 SQLite 字段"]
  B --> F["写入摘要 / 补问 / 运行日志"]
```
...
- 当前 POC 不做高风险字段拦截，只验证“Claude 能判断出来就直接写”
...
当前实验分支已经不只是文档设计，已经有一套可运行的本地 POC：

- 本地数据库：`SQLite`
- 数据位置：`experiments/agent-worker/data/agent-poc.sqlite`
- Agent Runner：逐步请求模型，每次只执行一个工具动作
- 本地工具：读档案、写 profile/intention/trait、写摘要、建补问、运行 SQL、finish
- 本地样本：`experiments/agent-worker/fixtures/demo-transcript.txt`

当前已经验证通过的能力：

1. 用一段 transcript 启动一次 Agent run  
2. Agent 分 6-7 步完成更新  
3. 基础字段写入 `profiles`  
4. 婚恋意图和择偶条件写入 `intentions`  
5. 兴趣、沟通风格、节奏、顾虑写入 `trait_profiles`  
6. 会话摘要写入 `conversations.ai_summary`  
7. 补问任务写入 `followup_tasks`  
8. 整个 run 会记录到 `agent_runs` 和 `agent_run_events`
...
- `npm test` 通过
- `npm run build` 通过
- `npm run agent:poc:init && npm run agent:poc:demo && npm run agent:poc:inspect` 通过

这说明本地 POC 已经证明了一件事：

**不用“大 JSON 合同”，Agent 也能分步骤把 transcript 写进数据库。**
