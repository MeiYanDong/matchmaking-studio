...
export function ProfileCard({ profile, intention, compact = false, printable = false }: ProfileCardProps) {
  const genderColor = profile.gender === 'male'
    ? 'bg-blue-100 text-blue-700 dark:bg-blue-400/14 dark:text-blue-200'
    : 'bg-pink-100 text-pink-700 dark:bg-pink-400/14 dark:text-pink-200'
        ...
        <div className="grid lg:grid-cols-[220px_1fr]">
          <div className="border-r border-gray-100 bg-gray-50 dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(14,20,30,0.98),rgba(10,15,22,0.98))]">
            ...
            <div className="p-4 space-y-3">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="dark:bg-white/[0.08] dark:text-foreground/78">{STATUS_LABELS[profile.status]}</Badge>
              </div>
              {printable && (
                <Button type="button" variant="outline" className="no-print w-full dark:border-white/12 dark:bg-white/[0.05] dark:text-foreground/78 dark:hover:bg-white/[0.08]" onClick={() => window.print()}>
                  <Printer className="w-4 h-4 mr-2" />
                  打印 / 导出
                </Button>
              )}
              {profile.raw_notes && (
                <details className="rounded-xl border border-amber-100 bg-amber-50 p-3 dark:border-amber-300/12 dark:bg-amber-400/[0.08]">
          ...
          <div className="p-6 space-y-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {intentLabel && (
                    <Badge className="bg-rose-100 text-rose-700 hover:bg-rose-100 dark:border-rose-300/12 dark:bg-rose-400/[0.08] dark:text-rose-100">
              ...
              {profile.appearance_score && (
                <div className="rounded-2xl bg-rose-50 px-4 py-3 text-center dark:border dark:border-rose-300/12 dark:bg-rose-400/[0.08]">
            ...
            {profile.hobbies?.length ? (
              <section className="rounded-3xl border border-rose-100 bg-rose-50/40 p-5 dark:border-rose-300/10 dark:bg-rose-400/[0.06]">
                <h4 className="mb-3 font-medium text-gray-900 dark:text-foreground">兴趣爱好</h4>
                <div className="flex flex-wrap gap-2">
                  {profile.hobbies.map((hobby) => (
                    <span key={hobby} className="rounded-full border border-rose-200 bg-white px-3 py-1 text-xs text-rose-700 dark:border-rose-300/12 dark:bg-white/[0.08] dark:text-rose-100">
                      {hobby}
                    </span>
                  ))}
                </div>
            ...
            <section className="space-y-3 rounded-3xl border border-gray-100 bg-white p-5 dark:border-white/10 dark:bg-white/[0.04]">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {intention.dealbreakers.map((item) => (
                      <span key={item} className="rounded-full border border-red-100 bg-red-50 px-3 py-1 text-xs text-red-600 dark:border-red-300/12 dark:bg-red-400/[0.08] dark:text-red-100">
                        {item}
                      </span>
                    ))}
                  </div>
...
function InfoRow({
  ...
  return (
    <div className="rounded-2xl border border-gray-100 bg-gray-50/70 px-4 py-3 dark:border-white/10 dark:bg-white/[0.05]">
