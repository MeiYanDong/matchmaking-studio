...
13. 输出务必精简，避免 JSON 过长被截断：
   - processing_notes 最多 4 条，每条不超过 30 个字
   - review_required.reason 尽量一句话
   - summary_updates 中的文本字段尽量控制在 60 个字以内
   - ai_summary 尽量控制在 120 个字以内
14. V1 不要把 relationship_mode、accepts_mode_compensated_dating、accepts_mode_fertility_asset_arrangement 当作默认关键缺口或默认补问。只有客户主动明确谈到这些特殊模式时，才允许作为 field_updates 或 review_required 输出；未提到时不要列入 missing_critical_fields。
...
missing_critical_fields 应优先覆盖通用建档与婚育边界字段，例如：
- 婚史
- 是否有孩子
- 是否接受对方婚史
- 是否接受对方有孩子
...
export function buildExtractPrompt({
  ...
  return [
    ...
    '所有文本字段都请尽量简洁，避免 JSON 因过长被截断。',
    'processing_notes 最多 4 条，每条尽量控制在 30 个字以内。',
    'ai_summary 尽量控制在 120 个字以内。',
  ].join('\n')
}
